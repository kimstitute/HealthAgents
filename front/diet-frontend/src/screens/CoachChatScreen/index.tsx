...
// API 함수
import { fetchChatResponse, initPlan } from "../../api/chat";
...
const handleSend = async (e: FormEvent) => {
  ...
  if (questionStep === 3) {
    ...
    const allUserData = {
      ...
      basicInfo,
      lifestyle,
      followup: updatedAnswers,
      device_id: undefined,
    };

    setIsSending(true);
    try {
      const planResponse = await initPlan(allUserData);
      ...
      const welcomeMsg: ChatMessage = {
        ...
        blocks: [
          {
            id: `bot-plan-init-${baseId}`,
            type: "markdown",
            content: "✅ 정보가 저장되었습니다! 이제 건강 데이터를 분석하여 맞춤형 조언을 드릴게요. 궁금한 점을 물어보세요!",
          },
        ],
      };
      setMessages((prev) => [...prev, welcomeMsg]);
    } catch (error) {
      console.error("Plan Init Error:", error);
  ...
  setIsSending(true);
  try {
    const response = await fetchChatResponse(text, undefined);
    const botMsg: ChatMessage = {
      id: baseId + 1,
      sender: "bot",
